...
{
    Serial.begin(BAUDRATE); // pour le debug
    ...
    Robot::setIntegralSaturation(2); //Si asservissement intégral : tuto choix valeur sur la def de la fonction
}
...
void Robot::setDebugMode(byte mode, bool debugAsserv)
{
    m_debug_asservissement=debugAsserv;
    // à asservir en temps
   // rajout du mode total
    m_debugMode = mode;
    if (mode ==2) debug();
}
