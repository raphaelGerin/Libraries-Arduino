...
{ 
  ...
  moteurDroit.setPID(8.387, -7.008, 0); //(1.429,-1.346,0);
  moteurGauche.setPID(5, -4.129, 0); //(0.9013, -0.826,0);
  ...
  attachInterrupt(digitalPinToInterrupt(codeurDroite2), motorCodeurIncrementalDB, CHANGE);
    monRobot.setAngleCorrecteur(3, 0, 0);
  monRobot.setDistanceCorrecteur(0.33, 0, 0);
  monRobot.setAngleSvrPtCorrecteur(1, 0, 0.33); //1, 0, 0.33);
  monRobot.setRapportAvancerTourner(0.01);
  monRobot.setPeriode(5);
  monRobot.setAcceleration(0.15);
  monRobot.setDebugMode(1); // 1 pour avoir le nom des données ; 2 sinon
